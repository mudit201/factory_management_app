    ...
    <div class="modal">
      ...
      <form @submit.prevent="handleSubmit">
        <VLayout style="justify-content: space-between;">
          ...
          <VSearchableSelect label="Product" placeholder="Select product...." v-model="selectedProduct">
            <VOption v-for="(product, index) in productList" :key="index" :text=product.label :value="product.value"></VOption>
          </VSearchableSelect>
...
import { VTextField, VSearchableSelect, VOption, VTimePicker, VButton, VLayout, VProgressRing,VBanner } from '@vonage/vivid-vue';
...
const end_time = ref('');
const operator_name = ref<string | undefined>(undefined);
// const operator_name = ('');
...
const handleSubmit = (async ()=> {
  ...
  const res = await batchStore.addBatch(data.value as BatchInput);
  let msg;
  let isError = false;
  if(error.value){
